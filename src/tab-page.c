...
static void close_unsaved_tab_response(AdwMessageDialog* dialog, GAsyncResult* result, AdwTabPage* target_page);
...
gboolean close_tab_page(AdwTabView* tab_view, AdwTabPage* page) {
    ...
    if(editor_buffer_get_edited(buffer)){
        ...
        adw_message_dialog_choose(ADW_MESSAGE_DIALOG(dialog), NULL, (GAsyncReadyCallback)close_unsaved_tab_response, page);
    }
...
static void close_unsaved_tab_response(AdwMessageDialog* dialog, GAsyncResult* result, AdwTabPage* target_page) {  
    ...
    EditorBuffer* buffer = page_get_buffer(target_page);

    // save response
    if (strcmp(response, "save") == 0) {
        // calls tab_view.close_page_finish(true) if file was successfully saved
        server_remove_tab(target_page);
        editor_buffer_save(buffer, true);
    }
    // close response
    else if (strcmp(response, "close") == 0) { 
        server_remove_tab(target_page);
        adw_tab_view_close_page_finish(tab_view, target_page, true);
    }
    // cancel response
    else if (strcmp(response, "cancel") == 0)
        adw_tab_view_close_page_finish(tab_view, target_page, false);
...
static void client_user_typed(GtkTextBuffer* buffer, gpointer _) {
...
ClientPage new_client_tab(AdwTabView* tab_view, const char* title) {
    ...
    g_signal_connect(rtrn.buffer, "end-user-action", G_CALLBACK(client_user_typed), NULL);
